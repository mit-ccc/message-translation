...
from transformers import BertTokenizer, BertModel, BertConfig, RobertaModel, RobertaConfig, RobertaTokenizer
...
def main():
    ...
    nLayers, nDims = 12, 768
    # only take 100 occurances at most
...
#     tokenizer = RobertaTokenizer.from_pretrained(modelName, never_split=targets, max_length=510)
#     model = RobertaModel.from_pretrained(adapter_dir, output_hidden_states=True)
    # model = AutoModel.from_pretrained("vinai/bertweet-base", output_hidden_states=True)
    model = AutoModel.from_pretrained("./models/bertweet", output_hidden_states=True)
    tokenizer = AutoTokenizer.from_pretrained("vinai/bertweet-base", use_fast=False, never_split=targets, max_length=510)
...
#     model.load_adapter(adapter_dir)
    ...
    targets_ids = [tokenizer.encode(t, add_special_tokens=False, truncation=True) for t in targets]
    print(targets_ids)
    ...
    usages = {
        i2w[target]: np.zeros((1, nLayers * nDims))  # usage matrix, only store the average token embedding
...
#                 print("layers", len(layers)) # printint 13 here becase of the extra LM head layer
                usage_vector = np.concatenate(layers[:12])
                if curr_idx[lemma] < MAX_INSTANCE_NUM:
                    usages[lemma][0, :] += usage_vector

                curr_idx[lemma] += 1
                nUsages += 1
    
    # obtain the average token embedding
    lemma_keys = list(usages.keys())
    for lemma in lemma_keys:
        if curr_idx[lemma]:
            usages[lemma][0, :] /= curr_idx[lemma]
        else:
            del usages[lemma]
    ...
    with open(outPath, 'wb') as handle:
        pickle.dump(usages, handle, protocol=pickle.HIGHEST_PROTOCOL)
